...
const SideBarChat = ({listConversation,idConversation,chooseConversation,handleOpenListFriend}) => {
   ...
   useEffect(()=>{
      socket.on("return-typing",(data)=>{
         console.log("t",data)
         setMsgTyping(data)
         clearTimeout(mClearTimeout)
         mClearTimeout = setTimeout(() => {
            setMsgTyping(undefined)
         }, 2000);
      })
      return () => socket.off("return-typing");
   },[])
   return (
                        ...
                        >
                           <div className="tyn-media-group">
                              ...
                              <div className="tyn-media-col">
                                 ...
                                 <div className="tyn-media-row has-dot-sap" style={{height:'18px'}}>
                                    {item?.message?.content ?
                                       ...
                                       <>
                                          <p className="content">{item?.message?.sender === id ? `Bạn: đã gửi ${item?.message?.message_image?.length} ảnh`: `Đã gửi ${item?.message?.message_image?.length} ảnh`}</p>
                                          <span className="meta">{getDate(item?.message?.created_at)}</span>
                                       </>
                                    }
                                 </div>
